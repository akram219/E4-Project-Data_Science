...
def predict_sarimax():
    ...
    for file in files:
        try:
            ...
            if 'Consommation' not in df.columns or 'Heures' not in df.columns:
                return jsonify({
                    "error": "Colonnes requises manquantes",
                    "colonnes_disponibles": list(df.columns),
                    "solution": "Assurez-vous que le fichier contient bien les colonnes 'Heures' et 'Consommation'"
                }), 400

            # Nettoyage de la colonne Heures
            df['Heures'] = pd.to_datetime(df['Heures'], errors='coerce')
            heures_invalides = df['Heures'].isna().sum()
            ...
            df['Heures'] = df['Heures'].apply(lambda x: datetime.combine(datetime(2023, 1, 1), x.time()))
            df.set_index('Heures', inplace=True)
